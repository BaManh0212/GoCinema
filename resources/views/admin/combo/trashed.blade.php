                    ...
                    @forelse ($combos as $combo)
                        <tr>
                            ...
                            <td class="text-center">
                                <form action="{{ route('admin.combo.restore', $combo->id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    <button type="submit" class="btn btn-sm btn-outline-warning">♻️ Khôi phục</button>
                                </form>
                                <form action="{{ route('admin.combo.forceDelete', $combo->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Bạn có chắc muốn xóa vĩnh viễn Combo này không?');">
                                    @csrf
                                    @method('DELETE')
